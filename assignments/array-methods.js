...
// Scroll to the bottom of the list to use some advanced array methods to help the 
//event director gather some information from the businesses.
...
ticketPriceTotal.push(runners.reduce((accumulator, runner) => accumulator + runner.donation, 0));
...
// Problem 1: Need the emails of everyone to send them a notice about the race
let emails = [];
emails = runners.map(runner => runner.email);
console.log(emails);

// Problem 2: Want to know who donated more than 100 to give them a prize
let biggestDoners = [];
biggestDoners = runners.filter(runner => runner.donation > 100);
console.log(biggestDoners);

// Problem 3: Want to know how many shirts size S to produce
let sizeSmall = [];
let totalSmall = runners.reduce((accumulator, runner) => {
